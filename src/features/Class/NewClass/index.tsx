...
export function NewClassFeature({ schoolId }: Props) {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: { name: '', shift: '' as ClassShift, academicYear: String(new Date().getFullYear()) },
  });

  async function onSubmit(values: FormValues) {
    await createClass(schoolId, { name: values.name, shift: values.shift, academicYear: Number(values.academicYear) });
